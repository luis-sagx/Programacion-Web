...
function setName(nameControl, valor, idForm = null) {
    let elemento;
    if (idForm) {
        elemento = document.querySelector(`#${idForm} [name='${nameControl}']`);
    } else {
        elemento = document.getElementsByName(nameControl)[0];
    }
    if (elemento) {
        elemento.value = valor;
    } else {
        console.error("error en setName");
    }
}
...
function limpiarDatos(idFormulario) {
    ...
    for (let i = 0; i < elementosName.length; i++) {
        ...
        setName(elementoName, "", idFormulario); 
    }
}
...
async function fetchPost(url, tipoRespuesta, frm, callback) {
    try {
        ...
        if (!res.ok) {
            ...
            throw new Error(`Error en POST: ${res.status} - ${errorMsg}`);
        }
        ...
        alert(`Ocurrió un error en POST: ${e.message}`);
    }
}
